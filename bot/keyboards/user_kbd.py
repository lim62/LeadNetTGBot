...
def get_workpulse_kbd(i18n: TranslatorRunner, up: str = '', down: str = '', have_up: bool = True, have_down: bool = True) -> InlineKeyboardMarkup:
    kbd: list[list] = []
    if have_up:
        kbd.append([InlineKeyboardButton(text=i18n.btn.previous(), callback_data=up)])
    if have_down:
        kbd.append([InlineKeyboardButton(text=i18n.btn.next(), callback_data=down)])
