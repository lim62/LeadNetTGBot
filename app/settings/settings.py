...
async def start_clients(session_maker: async_sessionmaker, rstorage: Redis) -> None:
    ...
    for client in clients:
        asyncio.gather(schedule_starting(client=client, session_maker=session_maker))
    
async def schedule_starting(client: Client, rstorage: Redis) -> None:
    await asyncio.sleep(random.uniform(1, 10))
    await client.start(rstorage=rstorage)

async def add_client(data: str) -> None:
    ...
    try:
        await client.custom_start()
    except Exception as e:
        raise e()
    
async def send_code_client(session_maker: async_sessionmaker, data: str, code: int) -> None:
    client: Client = Client(
        name=data['phone'][1:],
        api_id=data['api_id'],
        api_hash=data['api_hash'],
    ...
    try:
        await client.custom_end_start(code=code)
